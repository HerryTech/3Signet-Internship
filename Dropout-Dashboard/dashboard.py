...
# Create the heatmap with a specified size
fig = px.imshow(
    correlation_matrix,
    text_auto=True,
...
# Update layout to make it larger
fig.update_layout(
...
# Exclude non-numerical columns and ensure no NaN values
numerical_data = filter_data.select_dtypes(include=['number']).dropna()
...
# Check if the correlation matrix is computed successfully
if not correlation_matrix.empty:
    ...
    st.write("Correlation matrix could not be computed. Check data.")
